...
ESP32_IP = "192.168.100.158"
PORT = 8080
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

def send_command(signal_id, state, duration=None):
    state = state.upper()
    lane = signal_id + 1
    if duration is not None:
        message = f"{lane},{state},{int(duration * 1000)}" 
    else:
        message = f"{lane},{state}"             
...
def update_signals():
            ...
            if signal_timers[i] == 0:
                if signal_states[i] == 'yellow':
                    if allocation_time[i] > 0:
                        ...
                        send_command(i, 'green', allocation_time[i])
                    else:
                        signal_states[i] = 'red'
                        send_command(i, 'red')
                    ...
                    allocation_time[i] = 0
                    send_command(i, 'yellow', int(YELLOW_TIME))
    ...
    if all(signal_states[i] != 'green' and signal_states[i] != 'yellow' for i in range(noOfSignals)):
        ...
        if next_green is not None and green_time > 0:
            ...
            send_command(next_green, 'yellow', int(YELLOW_TIME))
